...
@section('container')
    ...
    <div class="row justify-content-center mb-3">
      <div class="col-md-6">
        <form action="/posts">
          {{-- jika didalam request domain ada category --}}
          @if (request('category'))
    ...
    <div class="card mb-3">
      @if ($posts[0]->image)
        <img src="{{ asset('storage/' .$posts[0]->image )}}" class="img-fluid" style="height:500px;" alt="{{ $posts[0]->category->name }}">
      @else
      <img src="{{URL::asset('/img/code.jpg')}}" class="card-img-top" style="height:500px;" alt="{{ $posts[0]->category->name }}">
      @endif
        ...
        <div class="card-body text-center">
          ...
          <p>
            <small class="text-muted">
                ...
                {{ $posts[0]->author->name }}</a> in <a href="/posts?category={{ $posts[0]->category->slug}}" class="text-decoration-none">
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="card">
                ...
                @if ($post->image)
                <img src="{{ asset('storage/' .$post->image )}}" class="img-fluid" style="height:300px;" alt="{{ $post->category->name }}">
                @else
                <img src="{{URL::asset('/img/code.jpg')}}" class="card-img-top" style="height:300px;" alt="{{ $post->category->name }}">
                @endif
               
                <div class="card-body">
                  <h5 class="card-title"><a href="/posts/{{ $post->slug }}" class="text-decoration-none text-dark">
                    {{ $post->title }}</a></h5>
